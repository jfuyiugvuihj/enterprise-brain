...
**不做什么**：见 §10。
**并行分工**：三条对话线（前端主干 / 前端叶子 / 后端）的独占文件集与对话内子 Agent 拆分规则见 `handoff/2026-09-15-frontend-parallel-tracks.md`；净收益约 2–3 天，只有 V5 原语 + V6 基建那块值得并行。**要开工直接用 `handoff/2026-09-15-frontend-startup-prompts.md`（A/B/C/D 四条对话线 + 子 Agent 模板，基线 `13e808d`）。**
