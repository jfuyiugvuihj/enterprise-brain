...
| P-9 | **语料在位**（09-18 22:0x 那轮废跑的根因，本班补） | `GET /api/v1/documents?page=1&page_size=500`（Bearer + `trust_env=False`）数 `documents[]` 条数，并确认 `制度与口径登记表.txt` 在列 | 篇数 == 预期 **100**（99 篇参考集 + R66 `9f2f869` 强制入库的登记表；`data/报销明细表.csv` 另计），**缺一即停**：语料不在位跑出来的准确率、证据覆盖率、无出处题数**全部作废**。🔴 删除/对账**禁止**拿宿主 `chroma_db` 快照当真相源——那份快照早于 R66，照它删就会删掉真实语料（第二十班前一班就是这么误删的，见看板 §4AZ.3） 🔴 **R201 口径更正（判据不变，只改读数解释）**：这条命令打的是 `app/api/v1/chat.py::list_documents`，而这枚路由**不声明任何查询参数**——`?page=1&page_size=500` 被框架丢弃（契约原话 `discarded by the framework, not honoured`，见 `docs/api/contract-v1.md` 的 `## Document Catalog Visibility`）⇒ 本行数到的 `documents[]` 条数**从来就是全量**，不是「500 条里的第一页」，把那两个参数删掉读数一字不变；判据仍是篇数 == 预期 **100**、缺一即停。 |
...
- **并发闸门不许动**：`MODEL_MAX_CONCURRENCY` 必须为 `1`（`docker-compose.yml:141` 默认；实现 `app/common/model_budget.py:53`、`:101`）。调高它跑出来的 P95 不是产品时延，是排队时延。另需注意（**09-21 本班实测订正**，旧句子「闸门与 n_ctx 都是镜像里的配置、改了 .env 不重建镜像等于没改」过宽，特此改窄）：`MODEL_MAX_CONCURRENCY` 走 `docker-compose.yml` 的 `environment:`、`deploy/.env.server` 走 `env_file:`，两枚都在**建容器那一刻**解析，与镜像内容无关 ⇒ 改这些值要的是 `up -d` **recreate**，不是 build。只有改 `app/common/model_budget.py` 里的 `DEFAULT_*` 常数才需要重建镜像（`GIT_SHA`/`BUILT_AT` label 同理走 build args）。实测背景：`MODEL_CONTEXT_TOKENS`/`MODEL_CONCURRENCY_WAIT_SECONDS`/`MODEL_MIN_ANSWER_TOKENS` 三枚**既不在 `deploy/.env.server` 也不在 compose 也不在镜像里**（`docker run --rm --entrypoint sh enterprise-brain:local -c "ls -a /app"` 无 `.env`；`docker compose run --rm --no-deps --entrypoint env backend` 现场只打印 6 枚 MODEL/CTX 变量），所以真机现在跑的是**代码默认 4096 / 60 s / 1536**；`load_dotenv()`（`app/agents/nodes.py:13` 等）默认不覆盖 `os.environ`，裸跑时才读仓库 `./.env`。⇒ 本条 P-8 的**镜像同源**要求不受影响（它测的是 `app/**` 代码与 label），但这三枚档位别再记成「要重建才生效」。详见跟进单 §70。
