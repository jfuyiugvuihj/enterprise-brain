...
| `Riemann`（昵称，唯一键是 agent_id） | 待补 | **R116** 把 46 枚装箱参数化钉桩升级成按**实测** `prompt_tokens` 复算 room + 装箱自身开销拆账（判据 跟进单 §54/§57 + 🔴 §62 追加 run5 三笔） | `be-r116`（总控自 **`2f965c1`** 新建 `codex/be-r116`，**独占**，开工 0 脏项） | 🟡 本班 22:4x 单投（`spawn_agent` 一次、`message` 通道、**不带 model override**）。写域 `tests/test_r112_*` 那族参数化 + `scripts/perf_probe_*.py` + 新用例；🔴 禁碰 `app/agents/tools.py`(R122 已结案、留给复验)、`orchestrator.py`(R33 簇)、`retrieval_pipeline.py`、`chat.py`、`contracts.py`、`.env*`、`frontend/**`、`tests/fixtures/**`。基线两值 **2759/35**。不占 GPU | 09-20 22:4x |
| `Margulis`（昵称，唯一键是 agent_id） | 待补 | **R125** `--status --json` 补两枚普查字段（能力在 `rebuild_index.py:176 vector_census()`，入口没接）+ 订正 pgvector 方案 §8.6（判据 跟进单 §61） | `be-r125`（总控自 **`2f965c1`** 新建 `codex/be-r125`，**独占**，开工 0 脏项） | 🟡 本班 22:4x 单投（一次、单通道、不带 model override）。写域 `scripts/rebuild_index.py` + 新用例 + `docs/handoff/2026-09-17-pgvector-adoption-plan.md` **§8.6 那一段**（append/整段替换，不许动 §8 其余）；🔴 禁碰 `migrations/**`、真 `deploy/.env.server`、`app/**`。与 R116 零文件交集。不占 GPU | 09-20 22:4x |
