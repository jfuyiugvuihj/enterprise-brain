...
| `Sartre` | `01a0cc4d-03ee-7291-98a3-9bb1e5e266d5` | **R170** 只有一行表头的 CSV 能把数据面板点崩（判据全文 跟进单 §85 三） | **`be-r170`**（总控自 `839c344` 新建 `codex/be-r170`，**独占**；`frontend\node_modules` Junction 由总控建，建后实取 `dirty=0`） | **在途**（11:2x 单枚 `spawn_agent`，**不带 `model` 字段**，一 block 一次投递，零补投）。写域锁 `app/tools/excel.py` + `app/api/v1/data.py` + `DataPanel.vue` 画像那处防御 + 新 `tests/test_r170_*.py` + 新 `__tests__/r170-*.test.js`；🔴 禁入 `hitl/**` 与 `ApprovalPanel.vue`（R168 在写）、`ChatPanel.vue`、`theme.css`、`src/lib/**`、`src/router/**`、`chat.py`（R172 在写）。判据：先交两枚红钉 → 根因（有列无行 = 合法空表画像 / 真·零列才走既有错误信封，前后端两处都改）→ 两把常驻反证；`lint:colors` 148 不许涨、全量与 724 一枚不许掉 | 09-23 11:2x |
