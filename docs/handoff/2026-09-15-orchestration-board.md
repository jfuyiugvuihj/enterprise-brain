...
| `Boole` | `01a0b9fe-f3a7-7a90-8e98-991144196f10` | **R98** checkpointer 谎报降级为 MemorySaver（§41.1） | `be-r98`（基线 `9a5aaab`，**独占**） | **运行中**：09-19 22:4x 派出，投递一次成功（本线程 `spawn_agent` 可用，§4BD.0 的「全族不可用」就地作废）；系统昵称 `Faraday` | 22:08:44 |
| `Gibbs` | `01a0b9ff-f150-71b3-a548-2281e4a1a244` | **R99** analysis 档预算与天花板自相矛盾 ⇒ 超时拿离线回复冒充答案（§41.2） | `be-r99`（基线 `9a5aaab`，**独占**） | **运行中**：09-19 22:5x 派出，投递一次成功；系统昵称 `Descartes`（与 09-18 那条 R30 的 `01a0b27a-…` 只是重名，`agent_id` 唯一，勿混用） | 22:09:24 |
