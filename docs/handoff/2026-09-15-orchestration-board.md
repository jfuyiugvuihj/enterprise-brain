...
| `Laplace` | `01a0bf4f-5e22-7d90-915c-0dc0dbb68546` | **R123 甲案 `4b731eb`／R38 `2e6abc6`／R29 `791568c`（本班并树，施工 `62c734d`；四枚 sha 逐位吻合、本树全量 2946-40 总控亲跑与自报逐位相同）· R29 判负如实入账：思考税真机 A/B 比例 0.9981，『不迁腿』这个负结果并钉成契约** → **本班复用派 R31**（生成轮流式透传 + 片段边界规则；判据全文跟进单 §21 R31；阶段 A 门②『流式逐字无缺』开张至今从没验过，本单是第一次真验） | **`be-r29`**（独占，已 ff 到 `791568c`） | 🟡 运行中：09-21 14:5x `send_input` 一次（复用结案 agent，零新增名额）。写域 `app/agents/nodes.py`+`app/agents/orchestrator.py` + 新用例（行号自 R29 起已漂移 ⇒ 🔴 一律符号锚引用，禁留裸行号）；🔴 禁 `frontend/**`（业主自写前端计划，全封）、`chat.py`+`contract-v1.md`(Hooke/R142)、`app/rag/**`+`main.py`+`migrations/**`(Tesla/R46)、`conftest.py`(Wegener/R134)、`spans.py`(Erdos/R146)、`scripts/**`(Chandrasekhar/R145)、`observability.py`(R135·S1 只读)。判据③的合并策略必须在后端做，不许推给前端；n≥8 真机抓帧期间独占模型，跑前确认无跑分窗口。若 ②③ 做不到 ⇒ 如实退回写清差哪条，不许用『字段达标』糊 | 09-21 14:5x |
...
| `Erdos` | `01a0bf41-d506-7d51-8f29-681571766be2` | **R119 `aaafdc9`／R135·S1 `c770f1f`（本班并树，施工 `9ada3df`；两枚 sha 逐位吻合、本树全量 **2964-40** 总控亲跑＝基线 2946−1(Postgres 跳)+19 新件，主树复跑 +19 逐枚对账）→ **本班复用派 R146**（cached-token 论述与记账归真；题面＝Laplace/R29 交工具名上报的待办②：原生 done 帧真有 `prompt_eval_cached_count`、兼容腿非流式报过 `cached_tokens=257`、而**流式 /v1 帧不带 usage** ⇒ R38 那句『本机不报 cached』两处错） | **`be-r119`**（独占，已 ff 到 `791568c`） | 🟡 运行中：09-21 14:5x `send_input` 一次（复用结案 agent，零新增名额）。写域 `app/trace/spans.py` + 新测试件；🔴 禁 `nodes.py`/`orchestrator.py`(Laplace/R31)、`contract-v1.md`(Hooke/R142，cached 那句**只登记不改**、回执转总控)、`app/rag/**`+`main.py`+`migrations/**`(Tesla)、`conftest.py`(Wegener)、`scripts/**`(Chandra)、`frontend/**`、`docs/**`、评测集。不许为凑数写 0、不许估算冒充实测、帧证据引用 R29 已注入那一份禁止复制第二份；**若记账必须动 nodes.py ⇒ 停下回报，不许伸手**。本单结论是 R43 判据② 订正的直接输入 |
...
| `Wegener` | `01a0bf45-7196-7bf3-86b7-d4832591229a` | **R33 已结案并树 `9678d21`**（施工 `b35e10f`；本树 2884-40 亲跑与自报逐位相同；四笔异议已裁定，异议③ 由总控亲做 `stage_timing.py` 一行订正入库 `9cdbef2`）→ **本班复用派 R134**（Chroma 沙箱漏口；题面＝看板 §4BH.18 第六条） | **`be-r33`**（独占，reset 到 `9cdbef2`；reset 前实测除 `chroma_db` 外零脏项、领先主干 0 枚） | 🟡 运行中：09-21 11:4x 投。12:1x 追投一条**第二成因**（`Hooke` 实测）：`pytest <具体文件路径>` 时 `tests/conftest.py` 不在祖先链 ⇒ R53 沙箱未装，就地写脏 tracked `chroma.sqlite3`；裸 `import app` 同形。判据升级为「无论怎么起都不许写仓库 `chroma_db`」。 |
| `Hooke` | `01a0c167-7175-77b0-8f37-bf049429cfe8` | **R132 已结案并树 `1fc43b4`**、**R32 已结案并树 `8a91f4e`**（施工 `0cfd86a`；四枚 sha 逐位吻合／本树全量 **2977-40** 总控亲跑；判据②字面与 `test_r37:330` 矛盾已由总控订正，判据⑤前端选择器按假控件禁令**不交**、拆 R141）→ **本班复用派 R142**（契约错误码表 26 vs 枚举 29 + `idempotency_key_required` 裸码挂号 + 裸行号引用改符号锚；题面＝它自己登记的既存漂移） | **`be-r32`**（独占，重切基线 `8a91f4e`） | 🟡 运行中：09-21 14:3x 投，零新增名额。写域 `contract-v1.md`+`chat.py`+`test_error_code_vocabulary.py`+新件，另仅限注释里的行号锚。🔴 已点名禁止再犯：`pytest app/api/v1/chat.py` 这种带路径参数的跑法（会绕过 conftest 沙箱写脏 `chroma.sqlite3`）。 |
...
| `Chandrasekhar` | `01a0c18c-4282-7cc2-b956-bc4efef60d44` | **R130 已结案并树 `cdc5ead`**、**R76 已结案并树 `546a93b`**（施工 `d2df5af`；四枚 sha 逐位吻合／本树全量 **2929-40** 与自报逐位相同；发布事务顺序与拒发闸总控亲读核实；它动的 `pgvector-adoption-plan.md` 13 行经核与代码相符予以保留，并自本枚起收走执行层写 `docs/**` 的口子）→ **本班复用派 R145**（向量镜像**集合面**对账：只在 Chroma／只在 PG／两边都有 三数 + NULL 待回填按 R76 新语义判读 + embedding 口径分组 + 孤儿；🔴 零 Ollama，因为 `Laplace` 正在跑 R29 真机 n≥8 实测，同时打模型两边数全废） | **`be-r76`**（独占，重切基线 `8a91f4e`） | 🟡 运行中：09-21 14:3x 投。硬约束：不得写仓库 `chroma_db/`（PersistentClient 只读也落 WAL ⇒ 先拷 `%TEMP%` 副本）、不得写 PG、不得 `--apply`。 |
