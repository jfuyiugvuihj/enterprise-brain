...
- **二、🔴 R205b 整刀封存，V1 前不并树**——本班真正的信息量在这里。派工词（前任所写）的立论「图表/洞察两族 = 全表 `tool_calls=4` 那批、四发模型压一发」被施工层用原件证伪，总控独立复核两条全部成立：
  ...
  - 🔴 **真凶换人**：这两族最贵的一格是每条 doc 腿内部那一发**阻塞查询改写**（`app/rag/retrieval_pipeline.py` `QueryRewriter.rewrite`，默认 `full` 档恒发，R51 参考表定价 **41.581 s**）。它不在图上、`tool_calls` 不数它、`fast` 档另有硬闸。⇒ p95 下一刀砍这里，别再砍重派轮。
...
- **四、🔴 一枚原件级缺陷登记（不修，先立规矩）**：`docs/testing/answers-run6.jsonl.latency_ms` 与 `sidecar-run6.jsonl.wall_ms` 总控逐位复算 105 行，>5% 偏差恰 3 行——`data-04` 1,342,781.94 vs 35,064.5（38×）、`data-06` 29,265,911.25 vs 55,930.8（523×）、`data-07` 1,206,484.047 vs 186,854.4（6.5×）。**谁拿 `latency_ms` 复算 p95 会得到 data 1342.8 s / ALL 163.3 s 的假数**。⇒ run7 复算只许读 sidecar 的 `wall_ms`；采集器跨休眠计时缺陷另立新单（写域与 R215 同文件，等 R215 并完再动，不许撞自己未提交的并树）。
