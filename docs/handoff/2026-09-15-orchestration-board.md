...
| `Wegener` | `01a0bf45-7196-7bf3-86b7-d4832591229a` | **R118**（只读定策单）doc 腿每轮冷启动 ⇒ 跨轮失忆：修装配让子图真 resume，还是承认失忆并把注释/文档改对（判据 跟进单 §55 + 本单投递词；解锁图 §B 簇 1 把它排在 `R118 → R33 → R29 → R31 → R43` 最前） | `be-r118`（总控自 **`7736302`** 新建 `codex/be-r118`，**独占**，开工 0 脏项） | 🟡 **运行中**：09-20 22:5x 单投（`spawn_agent` 一次、单通道、**不带 model override**）。🔴 **唯一可写 = 新文件 `docs/handoff/2026-09-20-r118-subgraph-memory.md`**，`app/**`/`tests/**`/`contract-v1.md` 一律禁碰（本单不定工）。它手里有前任没有的东西：run3/run4/run5 三枚报告 + 逐题侧车，判据 3 要求它回答「**`多轮对话` 三枚报告钉在 0.4167 一动不动**是不是跨轮失忆」。留档 `%TEMP%\r114_probe\r114_orchestrator.patch`（12 680 B）与 `kept_test_r114_history_tool_pruning.py`（20 169 B）本班亲验在盘，要求它先核 sha256 再 `git apply --check`（不许真 apply）。不占 GPU | 09-20 22:5x |
