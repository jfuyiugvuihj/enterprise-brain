...
| `Laplace` | `01a0bf4f-5e22-7d90-915c-0dc0dbb68546` | **R123 甲案已结案并树 `4b731eb`**（代提交 `7e0374b`；`kind=hitl` 不再冒充终答，同 session 同 Bearer 走真名 `POST /api/v1/approve`；总控亲跑 47 passed 4 skipped）＋ 双写窗探针由它做完 → **本班复用派 R38 代码半**（计量列不再写零：原生腿漏读 `prompt_eval_count`，`input_tokens` 恒 None；判据全文跟进单 **§67 三**） | `be-r123`（已结案）→ **`be-r38`**（总控自 **`6f3777d`** 新建，**独占**，开工 0 脏项） | 🟡 **运行中**：09-21 `send_input` 一次（复用已结案 agent ⇒ 不占新名额）。写域 `app/common/model_handler.py` + `app/trace/spans.py` + `app/trace/store.py` + `app/agents/nodes.py` 的 usage 读取处 + 新用例；🔴 **绝不许估算 token**（§21 原禁令）、`cached_tokens` 只许归真不许改数；真机「抽查一问」那半归总控等窗口；禁碰 `orchestrator.py`(R33)/`app/rag/**`(R130)/`retrieval_pipeline.py`(R119) | 09-21 10:0x |
...
| `Erdos` | `01a0bf41-d506-7d51-8f29-681571766be2` | **R116 已结案并树 `3b8a2e1`**（施工提交 `a04c221`；总控亲跑其树全量 2797-36 复现、五枚 sha 逐位吻合、禁碰零命中、旧断言零删除）→ **本班复用派 R119**（历史预留常数 322 与那把 132 字冒称 400 字的假尺；判据全文跟进单 **§67 一**） | `be-r116`（已结案）→ **`be-r119`**（总控自 **`3b8a2e1`** 新建，**独占**，开工 0 脏项） | 🟡 **运行中**：09-21 `send_input` 一次（**复用已结案 agent ⇒ 不占新名额**）。写域只到 `retrieval_pipeline.py:584-591` 两枚常数 + `test_r112`/`test_r116` 相关用例 + `perf_probe_run5_ledger.py` 取数口径；🔴 已明令它**必须踩并正确处置自己埋的 `_reserve()==954` 绊线**（不许放宽/删/改比较符），且要把判据 2 那个「17/17 变实料」在新尺下**重算重报**；另要求它把判据 6 定性出的两处补线以 ≤5 行写成需求单交回（`retrieval_pipeline.py:766` / `tools.py:1019` 都不是它的写域） | 09-21 10:0x |
...
| `Wegener`（唯一键是 agent_id） | `01a0bf45-7196-7bf3-86b7-d4832591229a` | **R127 已结案并树 `fd6aa8e`**（施工提交 `181f11c`；四枚 sha 逐位吻合、6+22+14+13=55 passed 总控亲跑、定策纸与主树逐字节相同故不重复入库；🔴 它一笔跨单冲突救下契约段——初稿的「七前缀闭合清单」对 R126 后的主树已是假话，退回只改那枚 bullet，改后写成「族+代表词+etc.+指向 chat.py 五枚常量名」）＋ R118 定策单早已结案 → **本班复用派 R33**（历史裁剪改零模型；判据全文跟进单 **§67 二**） | `be-r118`（已结案）→ **`be-r33`**（总控自 **`fd6aa8e`** 新建，**独占**，开工 0 脏项） | 🟡 **运行中**：09-21 `send_input` 一次（复用已结案 agent ⇒ 不占新名额）。写域 `app/agents/orchestrator.py`（只 `:50`/`:342` 邻域）+ `app/memory/**` + 新用例；🔴 禁碰 `:198` 装配与 `:212-215` 的 `checkpointer=`（R128 的刀）、禁碰 `retrieval_pipeline.py`(R119)/`app/rag/**`(R130)/`model_handler.py`+`app/trace/**`(R38)/`contract-v1.md`(R132 在读)；它自己刚钉的 `test_r118` 两枚注释钉必须仍绿；已明令「零模型前提下若摘要语义必损，照实说给替代方案，不许假绿」 | 09-21 10:1x |
| `Hooke`（系统返回的昵称，唯一键是 agent_id） | `01a0c167-7175-77b0-8f37-bf049429cfe8` | **R126 已结案并树 `6554901`**（代提交 `c69e190`；新件 20 passed、本树全量 2778-36 亲跑、12/12 命中 0/93 误伤；它对判据③「四对近义题任何泛化规则都分不开」的异议登记为 **R131** 候选）→ **本班复用派 R132**（给 `contract-v1.md` 那段会话/记忆语义与 `_is_followup` 之间补一枚一致性钉——R127 收口时查明**全仓没有用例读那段散文**；判据全文跟进单 **§67 四**） | `be-r126`（已结案）→ **`be-r132`**（总控自 **`fd6aa8e`** 新建，**独占**，开工 0 脏项） | 🟡 **运行中**：09-21 `send_input` 一次（复用已结案 agent ⇒ 不占新名额）。写域**只有**新用例 `tests/test_r132_*.py`；🔴 禁改 `app/api/v1/chat.py` 与 `docs/api/contract-v1.md` 的任何字节（发现二者今日已不一致要**具名上报不许顺手修**）；词表一律走 ast 取真源、测试里一处不许手抄；变异实验按新规矩单进程串行 + 每步断言 restore sha + 头尾对照组 | 09-21 10:1x |
