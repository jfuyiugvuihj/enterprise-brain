...
| `Newton` | `01a0ce4f-2e30-72b1-88e5-8ae0f4c64949` | **R193**（本班新立）越权矩阵接回主干：`eebaadb` 两枚件 + 一处种子 + 13 格归因 | `be-r193`（总控自 `749b754` 新建 `codex/be-r193`，**独占**；`.venv` Junction） | 🟡 **在途**（21:0x 单枚 `spawn_agent`）。写域**只有** `tests/test_r159_cross_scope_matrix.py` + `tests/test_r163_matrix_teeth.py`；🔴 禁 `app/**`、`migrations/**`、`frontend/**`、`docs/**`（计划书 §6 C 行由总控落笔）。判据＝补 `"department": DEPT_A` 一处种子 + `EXPECTED_ATTRIBUTION` 逐格归因到 R176/R177/R178/R179/R180/R185/R186 + 两枚计数钉改新值但不许消失 + teeth 反证一枚不许削弱 | 09-23 21:2x 本班实取 |
| `Halley` | `01a0ce51-1bd5-7902-bc21-af50772e3695` | **R194**（R179 具名交回）两笔同形不落审计的拒绝 | `be-r194`（总控自 `749b754` 新建 `codex/be-r194`，**独占**） | 🟡 **在途**（21:1x 单枚 `spawn_agent`）。写域 `app/api/v1/chat.py` + 新 `tests/test_r194_*.py`；禁 `migrations/**`、`frontend/**`、`docs/**`。两格＝`_authorize_queue_task` 五枚拒绝出口（含 `:725` owner 不匹配那枚 403）+ `GET /documents`（`chat.py:3410`）平铺路由那张「有但不能看」假脸：🔴 先取证消费者，零消费者就停手报处置建议，删接口是业主动作；响应体形状一枚字节不许动 | 09-23 21:2x 本班实取 |
...
| `Ampere` | `01a0ce64-aba0-76f3-8d0c-214e16b36166` | **R196**（本班新立·**只读审计单**）run6 开窗前置体检 | `be-r196`（总控自 `b839617` 新建，**零写盘**） | 🟡 **在途**（21:2x 单枚 `spawn_agent`）。五格＝预演器四发 / 出处覆盖率枚数（旧账 55/105，若变即本单头条）/ 用 run2-5 真产物重算窗口预算 / 宿主与线上态只读（`standby-timeout-ac`、盘余、现役镜像 revision、Redis `answer:*` 计数）/ 🔴 `be-eval95` 已被清理删除 ⇒ 只给重建命令不执行。🔴 不许开窗、不许跑 105 题、不许 `pytest`、不许动容器生命周期 | 09-23 21:2x 本班实取 |
| `Fermat` | `01a0ce6c-2a71-7af2-9ee2-20decc029d57` | **R197**（本班新立）切会话后出处卡片那把「已记下」的锁被继承 | `be-r197`（总控自 `484536c` 新建，**独占**；`frontend\node_modules` Junction 指主树） | 🟡 **在途**（21:4x 单枚 `spawn_agent`）。病灶：`ChatPanel.vue:1212` 外层 `v-for` 按 `:key="i"` 下标挂身份，而同屏 `CacheFace:1258` 已按 `turnKey(msg, i)` ⇒ 卡片实例跨轮复用，R195 那枚按 filename 存的一次性锁跟到新会话。判据＝先在自己树上复现红用例（复现不出来就停手报告，不许顺手改）→ 按 CacheFace 的形状给 `SourceCard` 挂按轮 key（🔴 不许改外层 `:key="i"`、不许新造 key 函数、不许 random/时间戳）→ 补一枚「切会话锁不许跟着 filename 走」的钉 → 同形名单（`ChartViewer :key="ci"`、`steps :key="si"`）**只报不动**。硬门：npm test 955/47 只许加、lint 恒 148（0 errors）、build EXIT=0 | 09-23 21:4x 本班实取 |
