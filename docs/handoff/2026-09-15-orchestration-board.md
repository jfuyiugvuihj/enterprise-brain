...
| `Laplace` | `01a0bf4f-5e22-7d90-915c-0dc0dbb68546` | **R123 甲案已结案并树 `4b731eb`**、**R38 已结案并树 `2e6abc6`**（施工 `c81fbb5`；五枚 sha 逐位／定向 141 passed 亲跑）→ **本班复用派 R29**（思考税：把生成轮搬原生 `/api/chat` 腿；判据全文跟进单 §68 一） | **`be-r29`**（独占，基线 `2e6abc6`） | 🟡 运行中：09-21 10:5x `send_input` 一次。写域 `app/common/model_handler.py`+`app/agents/nodes.py`+`model_budget.py`+`model_config.py` + 新用例；🔴 禁 `chat.py`/`frontend`(Hooke)/`orchestrator.py`(Wegener)/`retrieval_pipeline.py`(Erdos)/`indexing.py`+`pg_store.py`(Chandrasekhar)/`retriever.py`(Tesla)；宿主 Ollama 只许只读打 `/api/tags`+`/api/chat`，禁 `ollama create/pull/delete`、禁 `docker`、禁跑评测。若迁移受阻 ⇒ 退派生模型方案但只交件不许建模型 | 09-21 10:5x |
...
| `Erdos` | `01a0bf41-d506-7d51-8f29-681571766be2` | **R116 已结案并树 `3b8a2e1`** → **R119 在途**（历史预留尺重排；判据全文跟进单 §67 一） | **`be-r119`**（独占，基线 `3b8a2e1`） | 🟡 运行中：09-21 10:0x `send_input` 一次（复用结案 agent 不占名额）。写域 `retrieval_pipeline.py:584-591` 两枚常数 + `test_r112`/`test_r116` 相关用例 + `perf_probe_run5_ledger.py` 取数口径；🔴 必须踩并正确处置自埋绊线 `test_r116_measured_room.py:166 _reserve()==954`（不许放宽/删/改比较符），判据 2 的「17/17 变实料」须在新尺下重算重报。新件实名 `tests/test_r119_reserve_ruler.py`。10:43 仍在写。另欠总控一份 ≤5 行补线需求单（`retrieval_pipeline.py:766` / `tools.py:1019` 都不在它写域） | 09-21 10:5x |
| `Tesla` | `01a0bf43-94dc-7e51-844d-bd9da3adfa51` | **R125 已结案并树 `db414e0`**（施工 `c034db6`；三枚 sha 逐位／定向 34 passed／本树全量 2773-36 亲跑） → **本班复用派 R46 后端半张单**（判据全文跟进单 §68 二） | **`be-r46b`**（独占，基线 `2e6abc6`；🔴 陈旧分支 `codex/be-r46`@`0ae3b1e` 与本单无关，勿并） | 🟡 运行中：09-21 10:5x `send_input` 一次。写域 新 `migrations/0011_*.sql` + `manifest.json` + 新 feedback 出口（`app/main.py` 只加一行）+ `app/rag/retriever.py` 先验 + 新用例；🔴 **前端半张单不在本单**（Hooke 在改聊天视图，R46 前端排 R32 之后）；要动已有表或 0012 一律停下上报排号（Chandrasekhar 在 `indexing.py`/`pg_store.py`） | 09-21 10:5x |
| `Wegener` | `01a0bf45-7196-7bf3-86b7-d4832591229a` | **R127 已结案并树 `fd6aa8e`** + R118 定策单早已结案 → **R33 在途**（历史裁剪改零模型；判据全文 §67 二） | **`be-r33`**（独占，基线 `fd6aa8e`） | 🟡 运行中：09-21 10:1x 投。写域 `orchestrator.py`（只 `:50`/`:342` 邻域）+ `app/memory/**` + 新用例；🔴 禁碰 `:198` 装配与 `:212-215` 的 `checkpointer=`（R128 的刀）、禁碰 `retrieval_pipeline.py`(R119)/`loader.py`+`pg_store.py`(R76 在改)/`model_handler.py`(R29 在改)/`contract-v1.md`。已写：`orchestrator.py`/`memory/__init__.py`/`summarizer.py` + 新 `tests/test_r33_zero_model_compression.py`、`tests/test_r33_history_guardrails.py`。10:44 仍在写 | 09-21 10:5x |
| `Hooke` | `01a0c167-7175-77b0-8f37-bf049429cfe8` | **R126 已结案并树 `6554901`**、**R132 已结案并树 `1fc43b4`** → **R32 在途**（三档 lane 进契约 + 前端选择器；判据 §G-3 + §68 一） | **`be-r32`**（独占，基线 `2e6abc6`；树内 `frontend/node_modules` 用 junction 共享主树，已验证 vite 可跑） | 🟡 运行中：09-21 10:2x 投（本班第二枚复用）。写域 lane 值集与 400 校验（新拒绝码须先查 `tests/test_error_code_vocabulary.py` 已批清单）+ `app/api/v1/chat.py`/`app/models/**` + `docs/api/contract-v1.md` + **`frontend/**`（业主 09-21 已授权动前端）**；🔴 明令不许撤 `contract-v1.md` 的三处 `gated: needs lane labels`、不许做假控件（选档若不改变任何行为就只交 400 校验 + 可读数）、禁改 `_FOLLOWUP_*`/`_is_followup`（R132 会咬）。10:39 重切基线、10:42 跑过 pytest、交付未落盘 | 09-21 10:5x |
...
| `Chandrasekhar` | `01a0c18c-4282-7cc2-b956-bc4efef60d44` | **R130 已结案并树 `cdc5ead`**（P1 NUL，施工 `c16ad15`；新件 sha 逐位／本树全量 2804-40 亲跑；总控代核 `scripts/rebuild_index.py` 未动 + PG 现值 985/0/none）→ **本班复用派 R76**（`chunk_vectors` 接进发布/回填链；判据全文 §68 三） | **`be-r76`**（独占，基线 `2e6abc6`） | 🟡 运行中：09-21 10:5x `send_input` 一次。写域只有 `app/rag/indexing.py`+`app/rag/pg_store.py` + 新用例 + pgvector 方案文档相关段；🔴 它上单留的「把具名拒绝码迁进 retriever 的 REASON_ 群」**不属本单**（`retriever.py` 是 Tesla 的）；禁 `migrations/**`（要新列即停下排号）；换 embedding 模型不许留半张脸 | 09-21 10:5x |
...
- **四、R135 立成五期（计划书 §5.2 已插行，判据全文 §70）**：S1 读数（把「跑在 4096 上」从猜变成读，只碰 trace/health + 新测试，🔴 禁碰五枚在途写域）／S2 原生腿带 `options.num_ctx`（🔴 排 **R29** 之后，因为落点就是 R29 正在搬的那条腿）／S3 七枚钉子按实测重烘（`test_r112`/`test_r116`/`test_r99`/`test_r30_context_limit_guard`/`test_r30_model_tiers`/`test_r100`/`test_r102`，🔴 禁放宽断言，须在 run6 前定案）／S4 并发可见性（第 2 个人不许再拿离线客套文案；🔴 不许顺手把闸门提到 2，runbook 红线仍在）／S5 容器化 `qwen3.5:9b` 的 8192/16384 实测（🔴 必须机器空闲跑，且注意跑的是容器里那枚 9b，不是宿主 `qwen3:4b`）。
