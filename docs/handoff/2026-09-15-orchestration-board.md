...
| `Erdos` | `01a0bf41-d506-7d51-8f29-681571766be2` | **R119 已结案并树 `aaafdc9`**（施工 `ac57024`；五枚 sha 逐位／本树 2875-40 亲跑）→ **本班复用派 R135·S1**（模型档位读数：把「跑在 4096 上」从猜变成读；判据全文跟进单 §70） | **`be-r119`**（独占，基线 `29d75b3`） | 🟡 运行中：09-21 12:0x `send_input` 一次（复用结案 agent，零新增名额）。写域 `app/api/v1/observability.py` + 新测试件；🔴 明列禁碰 `nodes.py`/`model_budget.py`/`model_handler.py`/`chat.py`/`main.py`/`app/rag/**`/`migrations/**`/`conftest.py`/`frontend/**`。判据要点：`effective` 与 `source`（env/default）必须分开报——今天三枚档位全靠代码默认、只有 `MODEL_MAX_CONCURRENCY` 走 compose，混成一个数就是把坑留给下一班。 |
