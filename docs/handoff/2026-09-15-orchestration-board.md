...
- 需要用户拍板：写「待决」，不许自己猜（当前待决：「停止 = 拒绝挂起动作？」、真机验收探针账号是哪几个、仓库根垃圾要不要 `.gitignore`）。
- **全局停写条件**：主树 `git status --porcelain -- frontend` 出现除 `browser_data_quick.js`、`login-reference.png` 之外的任何条目 → 说明有人又在不版本化的状态下写前端，全部停手先归零。
