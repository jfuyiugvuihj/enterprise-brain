...
| `Laplace` | `01a0bf4f-5e22-7d90-915c-0dc0dbb68546` | **R123 甲案施工**：评测道对 18 枚 HITL 题走正道 `/chat/approve` 取真终答进分母，侧车同时留批准前形态使 run3/4/5 旧口径可重算（判据 跟进单 §57 + 本单投递词） | `be-r123`（总控自 **`1ff8dbd`** 新建 `codex/be-r123`，**独占**，开工 0 脏项） | 🟡 **运行中**：09-20 22:5x 单投（`spawn_agent` 一次、单通道、**不带 model override**），树由总控补建（工单明令执行层不许自建 worktree）。写域 `scripts/eval_transport_ask_v2.py` + `app/quality/**` + 新 `tests/test_r123_*.py`；🔴 禁碰 `app/api/v1/chat.py`（只读）/`scripts/collect_evaluation_answers.py`/`tests/fixtures/**`/`tests/test_evaluation_report.py`/`deploy/**`/`docs/handoff/**`。**特批一次真机单题探针**（`approval-05`，挂起→批准→终答三段证据必交，口令值不许落盘）⇒ 🔴 **探针未完前总控不许 recreate 后端容器**（pgvector 双写窗第 4 步因此在排队）。不占 GPU 之外的资源 | 09-20 22:5x || `Tesla` | `01a0b311-9ebd-7cf3-90ce-7dd04c8cf17a` | **R44** 热集进程内检索索引 | `be-r37`（新建分支 `codex/be-r46` @ **`5ae7e45`**，**独占**） | **终态补记（第十三班）**：13:51:11 投（返回 `Missing required argument: message` 是**假报错**；按硬规矩先查 rollout 确认唯一落地，**未补投**）。写域 `app/rag/hot_index.py`(新)/`retriever.py`/`retrieval_pipeline.py`；硬门 = **pre-filter 必须先于热集**、热集条目必须带 scope/index 版本、关闭时行为逐条一致、**禁碰 `migrations/**`（0011 留给 R49 入库列）** | 13:51 |
...
| `Erdos` | `01a0bf41-d506-7d51-8f29-681571766be2` | **R116** 把 46 枚装箱参数化钉桩升级成按**实测** `prompt_tokens` 复算 room + 装箱自身开销拆账（判据 跟进单 §54/§57 + 🔴 §62 追加 run5 三笔） | `be-r116`（总控自 **`2f965c1`** 新建 `codex/be-r116`，**独占**，开工 0 脏项） | 🟡 **运行中**：09-20 22:39:17 单投（`spawn_agent` 一次、`message` 通道、**不带 model override**），投后硬证＝新 rollout `rollout-2026-09-20T22-39-17-01a0bf41…jsonl` 110 152 B 已落盘。基线两值本班收窗后亲跑 **2759 passed / 35 skipped / 104.55 s**。写域 `tests/test_r112_*` 那族参数化 + `scripts/perf_probe_*.py` + 新用例；🔴 禁碰 `app/agents/tools.py`(R122 已结案、留给复验)、`orchestrator.py`(R33 簇)、`retrieval_pipeline.py`、`chat.py`、`contracts.py`、`.env*`、`frontend/**`、`tests/fixtures/**`。基线两值 **2759/35**。不占 GPU | 09-20 22:4x |
| `Tesla`（第三枚同名，唯一键是 agent_id） | `01a0bf43-94dc-7e51-844d-bd9da3adfa51` | **R125** `--status --json` 补两枚普查字段（能力在 `rebuild_index.py:176 vector_census()`，入口没接）+ 订正 pgvector 方案 §8.6（判据 跟进单 §61） | `be-r125`（总控自 **`2f965c1`** 新建 `codex/be-r125`，**独占**，开工 0 脏项） | 🟡 **运行中**：09-20 22:41:12 单投（一次、单通道、不带 model override），硬证＝`rollout-2026-09-20T22-41-12-01a0bf43…jsonl`。⚠️ **本班撞上一条新环境事实：`collab spawn failed: agent thread limit reached`**——它不是「没产生身体」的参数错，而是并发名额满。本班按硬证流程确认该次失败投递**零新 rollout、`be-r125` 树 0 脏项**，再 `close_agent` 结掉已并树的 `Franklin`/`Leibniz` 腾名额，重投出去的就是这一枚。⇒ 新规矩：**名额满先结案已并树的旧 agent 再投，别乱补投**。写域 `scripts/rebuild_index.py` + 新用例 + `docs/handoff/2026-09-17-pgvector-adoption-plan.md` **§8.6 那一段**（append/整段替换，不许动 §8 其余）；🔴 禁碰 `migrations/**`、真 `deploy/.env.server`、`app/**`。与 R116 零文件交集。不占 GPU | 09-20 22:4x |
| `Wegener`（唯一键是 agent_id） | `01a0bf45-7196-7bf3-86b7-d4832591229a` | **R118 已结案**（只读定策单）→ **本班复用为 R127（= 乙-1 施工单）**：`orchestrator.py:201` 注释与事实同色 + `docs/api/contract-v1.md` 一段散文语义 + 第一枚 `checkpoint_ns` 形状钉（判据 跟进单 **§64 三**） | `be-r118`（总控自 **`7736302`** 新建 `codex/be-r118`，**独占**；与主树 `e4e8c48` 之间 `git diff --name-only` 只有四枚 `docs/**` ⇒ 代码恒等，故复用原树） | 🟡 **R127 运行中**（09-21 08:5x `send_input` 一次下发，**不占新名额**）。R118 交付已入库：`docs/handoff/2026-09-20-r118-subgraph-memory.md`（33 660 B / 工作副本 sha256 前 16 `48451f703c2f5545` 逐位核；🔴 blob 因 `autocrlf` 是 LF 33 414 B，复验只能对工作副本取哈希）。四条承重断言总控逐条亲验为真；**裁定＝乙案切两刀 → R127 派建 / R128 待批排 R116 后；甲案关闭留门**；它顺手挖出的改写腿缺陷本班立为 **R126**（`Hooke`）。🔴 本单禁碰 `:198` 装配与 `:212-215` 的 `checkpointer=`（那是 R128 的刀） | 09-20 22:5x |
| `Hooke`（系统返回的昵称，唯一键是 agent_id） | `01a0c167-7175-77b0-8f37-bf049429cfe8` | **R126** 追问改写腿两处缺陷：「上一问」取成当前问（`chat.py:1116` 先存 / `:1125` 才读 / `:693` 取 `[-1]`）+ 触发白名单只认七个前缀、12 枚多轮题丢 8 枚（判据 跟进单 **§64 二**） | `be-r126`（总控自 **`e4e8c48`** 新建 `codex/be-r126`，**独占**，开工 0 脏项） | 🟡 **运行中**：09-21 08:5x **一次** `spawn_agent`（首次投递撞 `agent thread limit reached`，经硬证**无身体**：`be-r126` 仍 0 脏项、零新 rollout ⇒ 先 `close_agent` 结掉已并树的 `Galileo` 腾额后重试一次成功，不算事故 #14 的两通道重复投递）。写域 `app/api/v1/chat.py` 两段 + **必须同步修** `tests/test_r109_rewrite_offline_guard.py:78-85` 那枚 autouse fixture + 一枚走真存读路由的新用例。🔴 禁碰 `orchestrator.py`/`contract-v1.md`（R127 在写） | 09-21 08:5x |
| `Sagan`（只读调查单，唯一键是 agent_id） | `01a0c169-1408-7fa1-8a76-f952fc820544` | **R129**（无编号风险，登记为调查单）评测集 **29 枚 `must_contain` 在 95 篇语料里搜不到出处** ⇒ 从「一个总数」变成逐枚具名清单 + 分档 + 每枚三选一处置（A 改题 / B 补语料 / C 显式扣口径），真机跑分前必须拍板的那笔账（口径 = `scripts/check_eval_evidence_coverage.py`） | 无工作树（**只读主树**）；🔴 唯一可写 = 新文件 `docs/handoff/2026-09-21-must-contain-orphans.md` | 🟡 **运行中**：09-21 08:5x `spawn_agent` 一次（复用 `close_agent` 结掉 `Cicero` 腾出的名额）。🔴 **明令禁跑 pytest / docker / 任何 HTTP / GPU**——此刻同机三枚 Agent 正在做实测计时，跑测试会污染它们的数；只允许 `check_eval_evidence_coverage.py` 与只读 python | 09-21 08:5x |
