...
> **⚠️ 状态（2026-09-15 标注）**：**本文件自述为「总方案草案」，从未定稿**（见 §1 文档定位）。
> **不得作为实现依据或完成度依据**。1226 行的目标态描述与当前源码差距很大，尤其洞察 / 图谱 / 审批三页——已裁定改名、撤入口、降级为子视图，见 `docs/frontend-plan-2026-09-14.md` §2。
> 现状唯一权威：`docs/current-functionality-2026-09-10.md` + 其修订记录。
...
| 11 | 企业应用集成与开放平台 | 内网 API、MCP 和受控企业应用接入（可选后置：不影响第一版验收，核心闭环全部交付后再启动） |
| 12 | 企业生产安全基线 | 核心安全项（权限、密钥、网络隔离、审计、备份）必做；增强项（恢复演练、监控深化等）可后置 |
| 13 | 全量联调与答辩演示 | 浏览器完整闭环、演示数据和验收报告 |
...
- 复测并定位全量测试阻塞点（此前观察到约 240 秒未结束，需先复现确认，不得当作已验证结论）；
- 旧阶段测试编号说明：仓库现存 `test_phase0_fixes.py` 至 `test_phase13_private_enterprise.py` 属于历史章节体系，其编号与本方案阶段 0-13 无对应关系，不得据此判断本方案各阶段的完成度。暂不重命名这些测试文件（避免破坏现有引用和历史记录）；本方案新增的阶段与测试统一使用“升级阶段 N / 功能名称”命名；
...
整合已存在的 `app/agents/planner.py`、`app/agents/critic.py`、`app/agents/contracts.py`（三者已有初步实现，尚未完成 orchestrator 接入和完整验收），将当前运行时的 Supervisor 模式升级为：
...
企业应用接入不能直接连接模型、数据库、Chroma 或文件目录，而应经过独立的企业应用接入网关：

```text
企业内部系统/消息平台
    -> 接入网关
    -> 身份映射、RBAC/ABAC、脱敏、限流、审计
    -> 企业智脑 API / MCP Gateway
    -> 私有模型、知识库、分析、洞察、驾驶舱、审批助手
```

外部应用只是受控入口，不能成为权限中心，也不能使用管理员身份代替真实用户。

### 18.2 接入模式

第一版支持以下模式：

1. 内网 API：面向 OA、ERP、CRM、财务系统和内部门户，默认推荐。
2. 内网 MCP：向内部 Agent 平台暴露受控工具，每个工具单独授权。
3. 消息平台适配器：支持企业微信、钉钉、飞书，但默认关闭。
4. 外部模型测试模式：仅使用脱敏或模拟数据，正式生产配置必须关闭。

公有云消息平台只能接收脱敏摘要、指标、图表和通知，默认不发送机密原文、完整聊天历史和未脱敏个人信息。

### 18.3 开放 API

```text
POST /api/v1/open/query
POST /api/v1/open/analyze
GET  /api/v1/open/insights
POST /api/v1/open/approval/preview
GET  /api/v1/open/dashboard/summary
```

每个接口必须配置客户端身份、用户映射、允许权限、部门范围、最高密级、脱敏规则、速率限制、输入输出大小和审计事件。
...
应用密钥只显示一次，支持轮换、禁用和过期。应用不能因为注册就获得全局权限。

外部调用必须支持 HTTPS、请求签名、时间戳、防重放、限流和审计。未注册应用、签名错误、过期请求和越权调用统一拒绝。
...
|---|---|
| L1 公开 | 可在受控外部应用中使用 |
| L2 内部 | 优先内网处理，必要时只发送脱敏摘要 |
| L3 敏感 | 只允许内网或企业专属环境 |
| L4 核心机密 | 只允许内网私有模型和授权人员 |

旧字段 `classification=1/2/3` 继续兼容，并映射到新的数据等级，不能因为转换而降低原有访问限制。
...
- 应用不能伪造用户或管理员；
- 不能绕过文档、数据、证据和图表权限；
- 不直接暴露 Ollama、数据库、Chroma 或文件端口；
- 外部平台关闭后核心内网功能仍可用；
- 所有调用可按应用、用户、资源和权限决策追踪。

## 19. 企业生产安全基线

> 本阶段拆分为两类：核心安全项（权限边界、密钥管理、网络隔离、审计、备份）属于正式生产部署的必要条件，必须完成；增强项（完整恢复演练、监控体系深化等）可在核心闭环全部交付后进行。

### 19.1 网络与部署

- Ollama、PostgreSQL、Chroma、Redis 和文件存储只监听内网；
- 前端只能通过反向代理访问 API；
- 外部回调进入隔离接入区，不能直接进入核心数据区；
- 测试和生产使用不同数据库、密钥、文件目录和模型配置；
- 管理后台限制来源网络或 VPN。
...
- 传输使用 HTTPS/TLS；
- 密钥和 Token 不进入 Git；
- 文件路径、原始文件名和资源 ID 做安全校验；
- 临时文件设置生命周期并定期清理；
- 日志、Trace、导出和外部响应都执行脱敏；
- 删除、权限拒绝、模型调用和外部应用调用都写入审计。

### 19.3 监控与恢复

监控 CPU、内存、磁盘、队列、模型耗时、错误率、权限拒绝、异常下载和外部应用调用频率。数据库、原始文件、语义目录、洞察、审批草稿和审计日志必须备份；Chroma 保存版本和元数据并支持重建。至少完成一次完整恢复演练。
...
代管 `app/memory/`（长期记忆与用户画像暂不升级，只保证现有引用不被破坏）。
...
同时负责现有 `app/api/v1/alerts.py` 的能力迁移与收敛：洞察体系与现有告警不长期并存，`alerts.py` 的去留在阶段 4 设计时确定；迁移完成前不得出现两套重复的异常判定逻辑。
...
7. 合并任何分支后立即运行该模块测试。
8. 未通过基线不得继续扩展下一个大阶段。
9. `app/rag/` 对外接口（retriever、retrieval_pipeline 的公共函数签名）变更需主集成 Agent 会签后方可合并。
...
## 26. 当前实施状态（2026-09-08）

### 已完成并验证

- 阶段 0：基线测试、离线模型回退、检索依赖回退、前后端启动和前端构建已验证；
- 阶段 1：RBAC/ABAC、权限拒绝、资源范围和审计专项测试已具备；
- 阶段 2：Planner/Critic/契约模块已接入现有编排路径，单轮和多轮浏览器问答已验证；
- 阶段 3：文档预览、来源证据、业务语义和数据文件上下文已验证；
- 阶段 4–8：洞察、驾驶舱、图谱、审批、Trace/质量模块及专项测试已具备；
- 阶段 10–12 的核心私有化、开放 API、签名校验和部署模板已具备；
- 阶段 13：登录、知识库上传/预览/删除、CSV 上传、单轮问答和多轮追问已通过真实浏览器验收；
- 最新回归证据：后端 `227 passed, 3 skipped`，前端 `npm run build` 通过。

### 已补齐

- 阶段 8：已新增 30 条正式业务评估集，覆盖十类场景，并支持分类正确率、证据覆盖率、无依据结论率和 P95 报告；
- 阶段 9：已补齐 SSE 心跳、客户端取消、请求级硬超时、取消接口、性能统计接口和 P95 报告脚本；
- 阶段 10/19：已补齐工作区备份、SHA-256 清单、路径穿越防护恢复、JWT 密钥轮换、递归日志脱敏和受保护健康详情；
- 阶段 11：企业应用集成与 MCP 的生产级接入仍属于可选后置，当前不阻塞第一版；
- 阶段 12：核心安全约束已完成，完整 PostgreSQL 灾备演练和生产监控接入需在目标部署环境执行。

以上状态以代码、测试输出和浏览器验收为准；后续 Agent 不得仅依据历史 `test_phase*` 文件名判断阶段完成度。
