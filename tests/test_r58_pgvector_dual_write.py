...
# ---------------------------------- 总控刀（orchestrator-owned）：判据② 的"读不出旧向量"三处闸门
# 承重点：双写开启时，删旧之前必须能原样读出旧向量；读不出来就一个字都不动地拒写。
# 这三条在 Curie 交工版本里没有测试（把 return None 换成空字典，18 条全绿），故由总控补写。


class _SnapshotReadFailure(FakeCollection):
    """按 ids 精确取旧行时抛错（等价于 chroma 读坏），按 where 找同名词条照旧。"""

    def get(self, ids=None, where=None, include=None):
        if ids is not None:
            raise MirrorError("injected chroma snapshot read failure")
        return super().get(ids=ids, where=where, include=include)


class _SnapshotLosesVectors(FakeCollection):
    """行读得出、向量少一条：放回时就是残缺，等于用一次"回滚"制造新的错位。"""

    def get(self, ids=None, where=None, include=None):
        result = super().get(ids=ids, where=where, include=include)
        if ids is not None and include is not None and result["ids"]:
...
def _stale_row(name):
    return _row(
        f"{name}_0",
        metadata={"filename": name, "chunk_index": 0, "hash": "old-hash"},
    )


def test_add_refuses_when_old_vectors_cannot_be_read(monkeypatch):
    """判据②：读不到旧向量快照 ⇒ 两腿一个字都不动地拒写，不是"先删了再说"。"""
    _scope_env(monkeypatch)
    stale = _stale_row("j.md")
    connection = FakeConnection(scope_row=_scope_row(), column_type=f"vector({DIM})")
    _wire(monkeypatch, connection)
    collection = _SnapshotReadFailure(rows=[stale], timeline=connection.timeline)

    with pytest.raises(retriever.VectorWriteRejectedError) as excinfo:
        _retriever(monkeypatch, collection).add_document("j.md", "brand new")
    assert excinfo.value.reason == retriever.REASON_VECTOR_MIRROR_UNAVAILABLE
    assert collection.rows == {"j.md_0": stale}
    assert "delete" not in connection.timeline
    assert "chroma:delete" not in connection.timeline
    assert "upsert" not in connection.timeline
    assert connection.commits == 0
    assert connection.rollbacks == 1
    assert connection.closes == 1


def test_add_refuses_when_snapshot_loses_vectors(monkeypatch):
    """判据②：行数与向量数对不上 ⇒ 同一道闸门，宁拒不动刀。"""
    _scope_env(monkeypatch)
    stale = _stale_row("k.md")
    connection = FakeConnection(scope_row=_scope_row(), column_type=f"vector({DIM})")
    _wire(monkeypatch, connection)
    collection = _SnapshotLosesVectors(rows=[stale], timeline=connection.timeline)

    with pytest.raises(retriever.VectorWriteRejectedError) as excinfo:
        _retriever(monkeypatch, collection).add_document("k.md", "brand new")
    assert excinfo.value.reason == retriever.REASON_VECTOR_MIRROR_UNAVAILABLE
    assert collection.rows == {"k.md_0": stale}
    assert "chroma:delete" not in connection.timeline
    assert connection.commits == 0


def test_delete_refuses_when_old_vectors_cannot_be_read(monkeypatch):
    """delete_document 同一条闸门：读不出快照就不许删，否则回滚时无货可放回。"""
    _scope_env(monkeypatch)
    stale = _stale_row("l.md")
    connection = FakeConnection(scope_row=_scope_row(), column_type=f"vector({DIM})")
    _wire(monkeypatch, connection)
    collection = _SnapshotReadFailure(rows=[stale], timeline=connection.timeline)

    with pytest.raises(retriever.VectorWriteRejectedError) as excinfo:
        _retriever(monkeypatch, collection).delete_document("l.md")
    assert excinfo.value.reason == retriever.REASON_VECTOR_MIRROR_UNAVAILABLE
    assert collection.rows == {"l.md_0": stale}
    assert "delete" not in connection.timeline
    assert connection.commits == 0
    assert connection.rollbacks == 1
