...
# ------------------------------------------------------------ 判据⑤ 迁移卫生
def test_the_migrations_stay_one_to_one_with_the_manifest_after_0013():
    """新增一版之后仍然一一对应；README 那条 fail-closed 规矩不许被本单弄破。

    尾号引信按 R58 先例连名带断言一起改口（0012 那枚由总控落笔，写域在本单之外；0013 那枚由
    R190 施工方本人改口，本班写域已含 migrations 尾号）：这是把钉子收紧一版，不是放宽。
    """
    ...
    assert not [name for name in on_disk if name.split("_")[0] > "0013"], "本单没排 0014 的号"
