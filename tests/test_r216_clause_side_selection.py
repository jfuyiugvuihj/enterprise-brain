...
登记料里同时躺着一份一般条款与一份只管某个范围的特例条款，一般条款与特例条款同场时，结论段却按一般条款作答 ⇒ 答反了。
🔴 **09-24 并树后总控订正（以本行为准）**：上面那句旧表述照抄了看板 §4BW 的归因「特例也被引到了」，
而本件判据① 的取证把它推翻了：`metric-08` 在 run6 的 6 枚证据是块 3/10/0/8/0/11，**特例所在的块 4 从没进 prompt**，
`退款不冲减销售额` 在证据与正文里 0 命中。⇒ 本单治的是「**两枚同场时必须选被点名侧**」这道作答纪律；
「名次里有、装箱没送」那一半**另立单**——不许拿本件冒充 `metric-08` 已救回。🔴 本件的每一枚断言都只讲"哪一侧算结论"，不讲"原话有没有进正文"——
判据① 与派工词都写明这两味收益不许互记。
