...
#: 告警台账里没有归属列（app/api/v1/alerts.py 的 _ensure 建的表只有
#: id/rule_id/message/ai_analysis/read/created_at），所以跨部门告警天然可达。
...
def _drive_alert(ctx: Ctx, cell: Cell) -> Receipt:
    ...
    alerts._MEM_ALERTS.append({"id": 1, "rule_id": 1, "message": FOREIGN_ALERT_MESSAGE, "read": False})
...
CELLS += [
    ...
    Cell(
        ...
        note="判据①的红（已裁遗留，不许洗白）：alerts 表一个归属列都没有（app/api/v1/alerts.py 的 _ensure 只建 id/rule_id/message/ai_analysis/read/created_at），所以任一部门的 manager 都能读到全公司的告警正文。告警设计件 §2 把这条记为 R1 裁定 (c) 的待裁残留：保持 staff 403、manager 可读全司。本件如实钉红，不因为已裁就免检。",
        finding="A",
        root_cause="product",
        evidence=(
